...
def transform():
    ...
    date_referencia = date(2020, 12, 31)

    # Calculates the difference in days between the hire date and the reference date
    days_diff = []
    for d in df_["hire_date"]:
        diff = date_referencia - d.date()
        days_diff.append(diff.days)

    # Transform the difference to years
...
def load():
    ...
    # Convert to parquet
    df_.to_parquet(
        "/tmp/time_in_company.parquet", index=False
    )
