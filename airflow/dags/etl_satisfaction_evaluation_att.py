...
def extract():
    ...
    df_.to_json("/tmp/employee_performance_evaluation.json", orient="records", lines=True
                )


def load():

    df_ = pd.read_json("/tmp/employee_performance_evaluation.json", orient="records", lines="True"
                       )
